...
func (suite *GatewayServiceTestSuite) TestGetTaskStatusPending() {
	...
	assert.Equal(suite.T(), task.StatePending.String(), resp.Status)
...
func (suite *GatewayServiceTestSuite) TestGetTaskStatusCompleted() {
	...
	err = suite.engine.TransitionTask(t.ID, task.StateScheduled)
	require.NoError(suite.T(), err)

	err = suite.engine.TransitionTask(t.ID, task.StateDispatched)
	require.NoError(suite.T(), err)

	result := []byte("email sent")
	err = suite.engine.CompleteTask(t.ID, result)
	require.NoError(suite.T(), err)
	...
	assert.NotNil(suite.T(), resp)
	assert.Equal(suite.T(), t.ID, resp.TaskId)
	assert.Equal(suite.T(), task.StateCompleted.String(), resp.Status)
...
func (suite *GatewayServiceTestSuite) TestGetTaskStatusFailed() {
	...
	err = suite.engine.FailTask(t.ID, errMsg)
	require.NoError(suite.T(), err)
	...
	require.NoError(suite.T(), err)
	assert.NotNil(suite.T(), resp)
	assert.Equal(suite.T(), task.StateFailed.String(), resp.Status)
	assert.NotNil(suite.T(), resp.Result)
